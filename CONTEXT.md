...
| **Lab** | This repo's QA cluster (Argo Workflows + KubeVirt on ghost). Additive to the Factory — lab data supplements factory data on the dashboard and is always marked as lab-sourced, never presented as factory health itself. |
...
| **Row contract** | The provenance shape every dashboard data row must carry: `source_url`, `collected_at`, `derivation`, `state`, `state_reason`. Defined in `docs/data/page-contracts.md`. Placeholders are `null` + `state: unavailable` — collectors never invent values. |
