...
| `thunderbolt0` interface absent entirely on `exo-0` (no `1-x` device under `/sys/bus/thunderbolt/devices/`, only host router `0-0`/`1-0`) after a reboot, forcing PCIe power/control to `on` has no effect | Physical USB4 cable link is down at the hardware layer, not a software/power-state issue — confirmed 2026-07-08 (thunderbolt kernel module loaded, domains present, but no remote device enumerated even after forcing `power/control=on` on bridge `0000:00:08.3` and controllers `c5:00.5`/`c5:00.6`) | Requires physical inspection: reseat the USB4 cable in Slot 1/4 on both `ghost` and `exo-0`. Cannot be fixed via SSH alone (`ghost` SSH is banned in this repo's policy) — needs the operator to physically check the cable, or a policy exception for one-time hardware diagnosis. Until confirmed, treat all "USB4 direct-attach East-West traffic" design work as **blocked on hardware**, not deployed. |
