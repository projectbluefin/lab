...
| `exo-0` has no `thunderbolt0` interface at all (`ip link show thunderbolt0` → `Device does not exist`), `/sys/bus/thunderbolt/devices/` shows only local host routers `0-0`/`1-0` (both `authorized=1`) with no downstream peer entry (e.g. no `0-1`), and ping to `ghost`'s previous USB4 link-local IP (`169.254.79.234`) loses 100% | **Not a bad cable — owner confirmed physical wire is fine (2026-07-08).** Drivers and power are fully configured on both sides: `thunderbolt_net` is loaded on both `ghost` and `exo-0`, and PCIe runtime PM is set to `on` (controllers awake). Peer link-down state points to port mapping (must use Slot 1 or Slot 4 for USB4) or BIOS/EC hardware state on `ghost`. | Once the physical link comes up, configure static IPs (`192.168.4.1/30` on `ghost`, `192.168.4.2/30` on `exo-0`) and apply kernel policy routing (`ip rule add to <peer-pod-CIDR> table 40`) to steer build-plane traffic over USB4 while keeping etcd/control-plane on Ethernet. See `docs/ghost-lab-architecture.md` for the routing design. |
