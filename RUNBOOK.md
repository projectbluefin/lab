...
| `exo-0` has no `thunderbolt0` interface at all (`ip link show thunderbolt0` → `Device does not exist`), `/sys/bus/thunderbolt/devices/` shows only local host routers `0-0`/`1-0` (both `authorized=1`) with no downstream peer entry (e.g. no `0-1`), and ping to `ghost`'s previous USB4 link-local IP (`169.254.79.234`) loses 100% | **Not a bad cable — owner confirmed the physical wire is fine on both nodes (2026-07-08).** `exo-0`'s own USB4 host router (`c5:00.6`, driver `thunderbolt`) enumerates correctly at boot (`journalctl -k` shows normal `ACPI: USB4 _OSC` + `usb usb4` init lines), but there is no XDomain/hotplug event afterward — i.e. `exo-0` never sees `ghost` show up as a remote Thunderbolt peer at the software level, even with the cable seated. This points at the **`ghost` side** of the link (wrong port, port not authorized, BIOS/EC state, or a software-side issue on that host) rather than the cable itself. Root-caused only from the `exo-0` side so far — a same-session read-only SSH probe to `ghost` returned `Connection refused`, so `ghost`'s own thunderbolt kernel/dmesg state could not be inspected. | Once `ghost` SSH is restored: check `journalctl -k | grep -i thunderbolt` and `/sys/bus/thunderbolt/devices/` on `ghost` for the same signature (host router present, no peer/XDomain entry). Confirm both ends are plugged into the certified USB4/TB4 port (not a plain USB-C port) and that BIOS has the Thunderbolt/USB4 controller enabled on `ghost`. Do **not** revisit `--flannel-iface=thunderbolt0`; until a peer is enumerated on both sides and the point-to-point ping succeeds, all "USB4 direct-attach East-West traffic" work remains **blocked on the `ghost`-side link**, not deployed. |
