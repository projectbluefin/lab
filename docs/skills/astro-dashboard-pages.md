...
85. When a page needs live data from Argo/k8s (LAN-only, never reachable from GitHub-hosted `ubuntu-latest` runners), do not invent a new schedule or bridge job. Add a job to the *existing* workflow that runs on the self-hosted `ghost-runners` ARC pool (`runs-on: ghost-runners`, see `AGENTS.md` "ARC Runners") and make the existing `ubuntu-latest` job `needs:` it with `if: always()` so the site still builds even if the self-hosted job is unavailable. Register the custom label in `.github/actionlint.yaml` (`self-hosted-runner.labels`) so actionlint doesn't false-positive on it. Use non-clobbering merges (`dict.setdefault(...)`) in the collector script so a later `ubuntu-latest`-only run never overwrites cluster data already committed by the `ghost-runners` job earlier in the same workflow run.
