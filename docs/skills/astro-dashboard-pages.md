...
85. When a page needs live data from Argo/k8s (LAN-only, never reachable from GitHub-hosted `ubuntu-latest` runners), do NOT assume `ghost-runners` (the ARC self-hosted Actions pool documented in `AGENTS.md`) is actually online — verify first with `gh api orgs/projectbluefin/actions/runners` and `gh api orgs/projectbluefin/actions/runner-groups`; if no runner carries that label, any workflow job with `runs-on: ghost-runners` will queue forever and never execute. The reliable bridge is an **Argo-native CronWorkflow** running in-cluster (e.g. `argo/workflow-templates/cluster-snapshot.yaml` + `manifests/cluster-snapshot-cron.yaml`): it already has LAN access to Argo/kubectl, needs no GitHub Actions runner at all, and can push its output straight to git using the existing `github-token` secret (already present in the `argo` namespace, already used read-only by `image-poller`/`pr-poller`/`pr-image-gc`/`pr-label-poller` - reuse it, don't mint a new credential). Follow the repo-wide convention of shipping new CronWorkflows with `suspend: true` and asking the user to `argo cron resume <name> -n argo` once reviewed, rather than defaulting to live. Use non-clobbering merges (`dict.setdefault(...)`) in the collector script so this job never fabricates or clobbers data structures other collector runs still need.
