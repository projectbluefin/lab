...
Dakota PR review is a lab-backed admission process. GitHub Actions status is advisory when the repository's known CI path is broken; the lab result is the merge gate for this workflow.
...
- The lab is authoritative for this operator flow when Dakota GHA checks are known to fail for unrelated CI reasons.
...
- [ ] If merged, the post-merge workflow was checked
