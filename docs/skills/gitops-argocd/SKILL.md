...
- **Missing `arc-github-secret` after ARC reinstall** — the `ghost-runners` scale set cannot create a listener or runner pods. Recreate the secret with `scripts/setup-arc-github-secret.sh` (requires gh CLI auth and a GitHub App private key generated in the App UI). The App ID and Installation ID are fetched automatically; do not hard-code them in docs or scripts.
...
Registry and base-image rules for lab images are documented in [image-policy.md](image-policy.md).
